...
locale basic_just_sp = 
  ...
  assumes just:
    "\<And>v. \<lbrakk>v \<in> verts G; v \<noteq> s; enum v \<noteq> \<infinity>\<rbrakk> \<Longrightarrow>
      \<exists> e \<in> arcs G. v = head G e \<and>
        dist v = dist (tail G e) + c e  \<and>
        enum v = enum (tail G e) + (enat 1)"

locale shortest_path_non_neg_cost =
  basic_just_sp +
  assumes s_in_G: "s \<in> verts G"
  assumes source_val: "dist s = 0"
  assumes no_path: "\<And>v. v \<in> verts G \<Longrightarrow> dist v = \<infinity> \<longleftrightarrow> enum v = \<infinity>"
  assumes non_neg_cost: "\<And>e. e \<in> arcs G \<Longrightarrow> 0 \<le> c e"
...
locale shortest_path_non_neg_cost_pred =
  basic_just_sp_pred +
  assumes s_in_G: "s \<in> verts G"
  assumes source_val: "dist s = 0"
  assumes no_path: "\<And>v. v \<in> verts G \<Longrightarrow> dist v = \<infinity> \<longleftrightarrow> enum v = \<infinity>"
...
sublocale shortest_path_non_neg_cost_pred \<subseteq> shortest_path_non_neg_cost
using shortest_path_non_neg_cost_pred_axioms 
by unfold_locales 
   (auto simp: shortest_path_non_neg_cost_pred_def 
...
lemma tail_value_helper:
  ...
  assumes "p \<noteq> []"
  shows "p = [hd p]"
by (metis assms distinct.simps(2) append_butlast_last_id hd_append
  append_self_conv2 distinct_butlast hd_in_set not_distinct_conv_prefix)
...
proof (rule ccontr)
  ...
  next
    show "\<mu> c s v = - \<infinity> \<Longrightarrow> False" 
    proof -
...
(*
text{*
  Proof by induction on enum v (for any v).
  Base case: trivial using assumptions.
  Step case:
   if it holds for (enum v) then it holds for (enum v)+1.
   using assumptions we know v~= s.
   By the induction hypothesis and the lemma dist_le_mu we know \dist(u) = \mu (c, s, u). 
   using just we obtain a witnessing edge (u,v) such that 
   dist(v) = dist(u) + c (u,v) = \mu (c, s, u) + c(u,v).
   we obtain the shortest path from s to u with cost \mu (c, s, u) 
   and append the edge (u,v) to it. It provides a path of cost \mu (c, s, u) +  c (u,v) from s to v. 
   Hence \mu(c, s, v) \<le> \mu (c, s, u) +  c (u,v) = \dist(v). By this we conclude our proof.
*}
*)
lemma (in basic_just_sp) dist_ge_\<mu>:
  ...
  assumes "dist s = 0"
  assumes "\<And>u. u\<in>verts G \<Longrightarrow> u\<noteq>s \<Longrightarrow> enum u \<noteq> enat 0"
...
lemma (in shortest_path_non_neg_cost) tail_value_check: 
  fixes u :: 'a
...
proof -
  ...
  have "(\<And>p. awalk s p s \<Longrightarrow> ereal(awalk_cost c p) \<ge> ereal 0)"
   using non_neg_cost pos_cost_pos_awalk_cost by auto
...
lemma (in shortest_path_non_neg_cost) enum_not0:
  fixes v :: 'a
  assumes "v \<in> verts G"
  assumes "v \<noteq> s"
  (*assumes "enum v \<noteq> \<infinity>"*)
  shows "enum v \<noteq> enat 0"
proof (cases "enum v \<noteq> \<infinity>")
case True
...
lemma (in shortest_path_non_neg_cost) dist_ne_ninf:
  fixes v :: 'a
  assumes "v \<in> verts G"
...
case False 
  ...
  proof(induct n arbitrary: v) 
  case 0 thus ?case 
    using enum_not0 source_val by (cases "v=s", auto) 
  next
  case (Suc n)
    thus ?case 
    proof (cases "v=s") 
    case True 
      thus ?thesis using source_val by simp
    next
    case False
      obtain e where e_assms:
        "e \<in> arcs G"
...
theorem (in shortest_path_non_neg_cost) correct_shortest_path:
  fixes v :: 'a
  assumes "v \<in> verts G"
  shows "dist v = \<mu> c s v"
  using no_path[OF assms(1)] dist_le_\<mu>[OF assms(1)]
    dist_ge_\<mu>[OF assms(1) _ dist_ne_ninf[OF assms(1)] 
    tail_value_check[OF s_in_G] source_val enum_not0] 
    by fastforce

corollary (in shortest_path_non_neg_cost_pred) correct_shortest_path_pred:
  fixes v :: 'a
  assumes "v \<in> verts G"
  shows "dist v = \<mu> c s v"
